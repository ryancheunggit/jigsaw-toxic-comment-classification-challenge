...
# taken from https://www.kaggle.com/anttip/wordbatch-1-3-3-fm-ftrl-lb-0-9812
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='training fm-ftrl on bag of words features and make predictions')
    parser.add_argument('--max_word_features', type = int, default = 20000, help = 'max number of words for word vectorizer')
    parser.add_argument('--max_word_ngram', type = int, default = 3, help = 'ngram range for word vectorizer')
    parser.add_argument('--max_char_features', type = int, default = 50000, help = 'max number of words for char vectorizer')
    ...
    with timer("creating kfold splits"):
        y = train[LABELS].values
        y_label_dist = train.progress_apply(
                lambda row: "-".join([str(label) for label in row[LABELS]]
            ), axis = 1)
        splitter = StratifiedKFold(n_splits = 5, shuffle = True, random_state = CV_SPLIT_SEED)
        folds = list(splitter.split(train, y_label_dist))
    ...
    with timer("Creating numerical features"):
        num_features = [f_ for f_ in train.columns
                        if f_ not in ["comment_text", "clean_comment", "id", "remaining_chars", 'has_ip_address'] + LABELS]
    ...
    with timer("Tfidf on word"):
        word_vectorizer = TfidfVectorizer(
            ...
            token_pattern=None,
            stop_words='english',
            ngram_range=(1, args.max_word_ngram),  # was (1 , 2)
    ...
    if args.save_prediction:
        out_dir = "../models/fm_ftrl-w-{}-{}-c-{}-{}".format(
    ...
    with timer("Scoring fm-ftrl"):
        losses = []
        losses_per_folds = np.zeros(len(folds))
        submission = pd.DataFrame.from_dict({'id': test['id']})
        for i_c, class_name in enumerate(LABELS):
            ...
            for n_fold, (trn_idx, val_idx) in enumerate(folds):
                ...
                losses_per_folds[n_fold] += score / len(LABELS)
                ...
                submission[class_name] += sigmoid(submission[class_name + "_temp"]) / len(folds)
        ...
        if args.save_prediction:
            oof_df = train[[f + "_oof" for f in LABELS]]
            oof_df.columns = LABELS
            oof_df.to_csv(out_dir + "/train_meta_probs_round_0.csv", index=False,float_format="%.8f")

submission.to_csv(out_dir + "/test_probs_5_bag_arith_mean_round_0.csv", index=False, float_format="%.8f")
